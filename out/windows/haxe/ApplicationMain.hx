...
class ApplicationMain
{
	...
	{
		lime.system.System.__registerEntryPoint("OmegaEngine", create);

		#if (js && html5)
		#if (munit || utest)
		lime.system.System.embed("OmegaEngine", null, 1600, 900);
		#end
		#else
		create(null);
		#end
	...
	public static function create(config):Void
	{
		...
		app.meta["build"] = "38";
		app.meta["company"] = "SidGames5";
		app.meta["file"] = "OmegaEngine";
		app.meta["name"] = "Omega 1.0.0.0";
		app.meta["packageName"] = "com.example.myapp";
		app.meta["version"] = "1.0.0.0";
	...
	macro public static function getEntryPoint()
	{
		...
		switch (Context.follow(Context.getType("omega.engine.Main")))
		{
			case TInst(t, params):
				...
				if (hasMain)
				{
					return Context.parse("@:privateAccess omega.engine.Main.main()", Context.currentPos());
				}
				...
				{
					Context.fatalError("Main class \"omega.engine.Main\" has neither a static main nor a constructor.", Context.currentPos());
				}

			default:

				Context.fatalError("Main class \"omega.engine.Main\" isn't a class.", Context.currentPos());
		}
...
@:keep @:dox(hide) class DocumentClass extends omega.engine.Main {}
#else
class DocumentClass
{
